...
parameters{
  ...
  vector[n_Treatment] log_alpha_t;
  vector[n_Treatment] log_mu_t;
  vector[n_Treatment] log_tau_t;
  
  /// Tank parameters
  vector[n_Tank] log_alpha_ta;
  ...
  /// Treatment parameters
  vector[n_Treatment] log_epsilon_t;
  vector[n_Treatment] log_lambda_t;
  vector[n_Treatment] log_theta_t;
  
  /// Tank parameters
  vector[n_Tank] log_epsilon_ta;
  vector[n_Tank] log_lambda_ta;
  vector[n_Tank] log_theta_ta;
}
...
model{
  ...
  //// Treatment parameters
  log_alpha_t ~ normal( log_alpha_mu , log_alpha_sigma_t );
  log_mu_t ~ normal( log_mu_mu , log_mu_sigma_t );
  log_tau_t ~ normal( log_tau_mu , log_tau_sigma_t );
  
  //// Tank parameters
  log_alpha_ta ~ normal( 0 , log_alpha_sigma_ta );
  ...
  //// Treatment parameters
  log_epsilon_t ~ normal( log_epsilon_mu , log_epsilon_sigma_t );
  log_lambda_t ~ normal( log_lambda_mu , log_lambda_sigma_t );
  log_theta_t ~ normal( log_theta_mu , log_theta_sigma_t );
  
  //// Tank parameters
  log_epsilon_ta ~ normal( 0 , log_epsilon_sigma_ta );
  log_lambda_ta ~ normal( 0 , log_lambda_sigma_ta );
  log_theta_ta ~ normal( 0 , log_theta_sigma_ta );
  ...
  //// Function
  // This parameterisation is better for large variance
